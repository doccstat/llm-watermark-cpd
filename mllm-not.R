...
for (models_folders_suffix_index in seq_along(models_folders_suffix)) {  # nolint
  models_folders_suffix_csv <-
    models_folders_suffix[models_folders_suffix_index]
  for (template_index in seq_along(pvalue_files_templates)) {
    ...
    for (prompt_index in seq_len(prompt_count)) {
      ...
      for (seeded_interval_index in seq_len(nrow(seeded_intervals))) {
        ...
        filename <-
          sub(".csv", paste0("-", models_folders_suffix_csv, ".csv"), filename)
        seeded_intervals_df[
